...
public class HttpClientUtilTest {
    ...
    @Test(priority = 10,
            description = "Test that fetchJson throws a VerificationClientException when the URL has invalid syntax")
    public void testFetchJsonWithInvalidUrlThrowsClientException() {

        // Execute test and verify
        Assert.assertThrows(VerificationClientException.class,
                () -> HttpClientUtil.fetchJson("not-a-url"));
    }

    @Test(priority = 11,
            description = "Test that fetchJson throws a VerificationClientException when a plain HTTP URL is used")
    public void testFetchJsonWithHttpSchemeThrowsClientException() {

        // Execute test and verify
        Assert.assertThrows(VerificationClientException.class,
                () -> HttpClientUtil.fetchJson("http://example.com/openid-configuration"));
    }
}
